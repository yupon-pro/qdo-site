  ...
  return (
    <Fragment>
      <Container
        maxWidth={false}
        sx={{
          backgroundImage: "url('/thirdparty/fukuoka.jpg')",
          backgroundSize: "100%",
      ...
      <Stack alignItems="center" spacing={5} sx={{ marginY: "2rem" }}>
        <Box>
          <Typography variant="h3"> Fukuoka</Typography>
        </Box>
        <Box sx={{ width: { xs: "75%", sm: "60%" } }}>
          ...
          <BodyTypography>
            ...
            scenic coastlines, lush mountains, and serene countryside. Visitors can explore picturesque seaside towns
            like Itoshima, renowned for its sandy beaches and stunning sunsets, or venture inland to discover tranquil
            hot springs and verdant valleys.
          </BodyTypography>
          <BodyTypography>
            ...
            offers a glimpse into Japan{"'"}s rich cultural heritage.
          </BodyTypography>
          <BodyTypography>
          ...
          <BodyTypography>
            With its perfect blend of tradition and modernity, natural beauty, and urban excitement, Fukuoka Prefecture
            captivates visitors with its endless charm and allure, making it a must-visit destination for travelers
            seeking an authentic Japanese experience.
          </BodyTypography>
        ...
        <ImageList sx={{ width: { xs: 400, md: 650 }, height: 1250 }} variant="quilted" cols={4} rowHeight={121}>
          {images.map((image) => (
            <ImageListItem key={image.title} cols={image.col || 1} rows={image.row || 1}>
              <PhotoDialog image={{ title: image.title, url: `/thirdparty/fukuoka/${image.title}.jpg` }} />
            </ImageListItem>
          ))}
        </ImageList>
      </Stack>
